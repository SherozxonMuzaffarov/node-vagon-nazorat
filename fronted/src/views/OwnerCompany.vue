<template>
    <div >
      <div class="d-flex align-items-center column-gap-2 mb-3">
        <p class="lead m-0">
          <i class="bi bi-house-door-fill"></i>Vagon firmasi egasi
        </p>
  ...
  const handleOk = async () => {
    try {
      let res = await axios.post('/owner-company/create', FormData.value);
      if (res) {
        getAll();
  ...
  const handleUpdate = async () => {
    ...
    try {
      let res = await axios.patch('/owner-company/update/' + id, FormData.value);
      if (res) {
        getAll();
  ...
  const deleteItem = async (id) => {
    try {
      ...
      if (confirmDelete) {
        try {
          let res = await axios.delete('/owner-company/delete/' + id);
  ...
  let getOne = async (id) => {
    try {
      let res = await axios.get('/owner-company/one/' + id);
      if (res.data) {
