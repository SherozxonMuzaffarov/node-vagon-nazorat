<template>
    <div >
      <div class="d-flex align-items-center column-gap-2 mb-3">
        <p class="lead m-0">
          <i class="bi bi-house-door-fill"></i>Ta'mir turlari
        </p>
  ...
  const handleOk = async () => {
    try {
      let res = await axios.post('/repair-type/create', FormData.value);
      if (res) {
        getAll();
  ...
  const handleUpdate = async () => {
    ...
    try {
      let res = await axios.patch('/repair-type/update/' + id, FormData.value);
      if (res) {
        getAll();
  ...
  const deleteItem = async (id) => {
    try {
      ...
      if (confirmDelete) {
        try {
          let res = await axios.delete('/repair-type/delete/' + id);
  ...
  let getOne = async (id) => {
    try {
      let res = await axios.get('/repair-type/one/' + id);
      if (res.data) {
